...
(setq imba-highlights
      '(("#\s+.+$" . (0 font-lock-comment-face))
        ("\\b\\([^0-9][A-Za-z_0-9]+\\)[\.]new" . (1 font-lock-type-face))
        ("[@][^0-9][A-Za-z_0-9]+" . (0 font-lock-variable-name-face))
        ...
        ("\\b\\(tag\\|def\\|class\\)[ \t]+\\([^0-9][A-Za-z_0-9]+\\)" . (2 'imba-function-name-face))
        ("\\btag[ \t]+[^0-9][A-Za-z_0-9]+[ \t]+\\([<][ \t]+[^0-9][A-Za-z_0-9]+\\)" . (1 font-lock-type-face))
        ("[<]\\(self\\)" . (1 font-lock-constant-face))
        ("[<]\\([A-Za-z_]+\\)" . (1 'imba-string-variable-name-face))
        ("[<]\\([^0-9][A-Za-z_0-9]+\\)" . (1 'imba-string-variable-name-face))
        ("[\'\"].+?[\'\"]" . (0 font-lock-string-face))
        ("\\([\.][^0-9][A-Za-z_0-9]+\\)" . (1 font-lock-preprocessor-face))
        ("\\([:][^0-9][A-Za-z_0-9]+\\)" . (1 font-lock-constant-face))
        ("\\([^0-9][A-Za-z_0-9]+[:\.][ \t]*\\)" . (1 font-lock-constant-face))
        ("\\([^0-9][A-Za-z_0-9]+\\)[:\.\(\[]" . (1 font-lock-variable-name-face))
        ("\\b\\(def\\|tag\\|var\\|let\\|const\\|prop\\|new\\|if\\|else\\|for\\|until\\|while\\|do\\|map\\|class\\|setTimeout\\|setInterval\\|clearTimeout\\|clearInterval\\|parseInt\\|parseFloat\\|__dirname\\|process\\|console\\|document\\|window\\|this\\|self\\)\\b" . (1 font-lock-function-name-face))
        ...
        ("\\b\\(var\\|let\\|const\\)[ \t]+\\([^0-9][A-Za-z_0-9]+\\)" . (2 font-lock-variable-name-face))        
        ))
